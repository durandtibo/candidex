...
def test_check_colorlog_with_package() -> None:
    with patch("candidex.utils.imports.colorlog.is_colorlog_available", lambda: True):
        check_colorlog()


def test_check_colorlog_without_package() -> None:
    with (
        patch("candidex.utils.imports.colorlog.is_colorlog_available", lambda: False),
...
def test_colorlog_available_without_package() -> None:
    with patch("candidex.utils.imports.colorlog.is_colorlog_available", lambda: False):
        fn = colorlog_available(my_function)
        assert fn(2) is None
