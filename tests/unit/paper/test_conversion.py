...
from candidex.columns import PAPER_ID, PAPER_TITLE, PAPER_URL, PAPER_VENUE, PAPER_YEAR
...
def make_paper(
    ...
    return Paper.from_raw(title=title, venue=venue, year=year, pdf_url=pdf_url)
...
def test_papers_to_dataframe_multiple_papers() -> None:
    paper_a = make_paper(title="Attention Is All You Need", venue="NeurIPS", year=2017)
    paper_b = make_paper(
        title="BERT", venue="NAACL", year=2019, pdf_url="https://arxiv.org/pdf/1810.04805"
    )
    assert_frame_equal(
...
def test_papers_to_dataframe_preserves_order() -> None:
    paper_a = make_paper(title="Charlie Paper", venue="CVPR", year=2020)
    paper_b = make_paper(
        title="Alice Paper", venue="ICLR", year=2021, pdf_url="https://arxiv.org/pdf/2.pdf"
    )
    paper_c = make_paper(
        title="Bob Paper", venue="NeurIPS", year=2022, pdf_url="https://arxiv.org/pdf/3.pdf"
    )
    assert_frame_equal(
