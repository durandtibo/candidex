r"""Evaluate the performance of `find_openreview_profile_url`."""

from __future__ import annotations

import urllib.parse
from concurrent.futures import ThreadPoolExecutor, as_completed
from dataclasses import dataclass

from dotenv import load_dotenv
from rich.console import Console
from rich.table import Table

from candidex.sandbox.affiliation import AuthorAffiliation
from candidex.sandbox.openreview import find_openreview_profile_url
from candidex.sandbox.progressbar import make_progressbar

console = Console()


@dataclass
class Sample:
    author: AuthorAffiliation
    profile_urls: list[str]

    def __post_init__(self) -> None:
        """Normalise all profile URLs on construction.

        Ensures that percent-encoded variants (e.g. '%7E' vs '~') are
        treated as equivalent during evaluation without requiring
        normalisation at comparison time.
        """
        self.profile_urls = [normalise_openreview_url(url) for url in self.profile_urls]

    @property
    def has_profile(self) -> bool:
        """Return True if the author is expected to have an OpenReview
        profile."""
        return len(self.profile_urls) > 0


def normalise_openreview_url(url: str) -> str:
    """Normalise an OpenReview profile URL for comparison.

    Decodes percent-encoded characters so that URLs like
    'https://openreview.net/profile?id=%7EThibaut_Durand1' and
    'https://openreview.net/profile?id=~Thibaut_Durand1' are treated
    as equivalent.

    Args:
        url: The OpenReview profile URL to normalise.

    Returns:
        The URL with percent-encoded characters decoded.

    Example:
        >>> normalise_openreview_url("https://openreview.net/profile?id=%7EThibaut_Durand1")
        'https://openreview.net/profile?id=~Thibaut_Durand1'
    """
    return urllib.parse.unquote(url)


def make_dataset() -> list[Sample]:
    """Create a dataset of authors to evaluate
    `find_openreview_profile_url`.

    Each sample contains an `AuthorAffiliation` with the author's name and
    a known affiliation, paired with the list of expected OpenReview profile
    URLs. Multiple URLs per sample account for authors who have more than
    one OpenReview profile. Multiple samples for the same author test
    robustness across different affiliation strings.

    Returns:
        A list of `Sample` instances for evaluation.
    """
    return [
        Sample(
            AuthorAffiliation(author="Thibaut Durand", affiliations=["LIP6"], email=None),
            profile_urls=[
                "https://openreview.net/profile?id=%7EThibaut_Durand1",
                "https://openreview.net/profile?id=%7EThibaut_Durand2",
            ],
        ),
        Sample(
            AuthorAffiliation(author="Thibaut Durand", affiliations=["SFU"], email=None),
            profile_urls=[
                "https://openreview.net/profile?id=%7EThibaut_Durand1",
                "https://openreview.net/profile?id=%7EThibaut_Durand2",
            ],
        ),
        Sample(
            AuthorAffiliation(
                author="Thibaut Durand", affiliations=["Simon Fraser University"], email=None
            ),
            profile_urls=[
                "https://openreview.net/profile?id=%7EThibaut_Durand1",
                "https://openreview.net/profile?id=%7EThibaut_Durand2",
            ],
        ),
        Sample(
            AuthorAffiliation(author="Thibaut Durand", affiliations=["Borealis AI"], email=None),
            profile_urls=[
                "https://openreview.net/profile?id=%7EThibaut_Durand1",
                "https://openreview.net/profile?id=%7EThibaut_Durand2",
            ],
        ),
        Sample(
            AuthorAffiliation(author="Thibaut Durand", affiliations=["RBC Borealis"], email=None),
            profile_urls=[
                "https://openreview.net/profile?id=%7EThibaut_Durand1",
                "https://openreview.net/profile?id=%7EThibaut_Durand2",
            ],
        ),
        Sample(
            AuthorAffiliation(author="Sepidehsadat Hosseini", affiliations=["SFU"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Sepidehsadat_Hosseini2"],
        ),
        Sample(
            AuthorAffiliation(
                author="Sepidehsadat Hosseini", affiliations=["Simon Fraser University"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Sepidehsadat_Hosseini2"],
        ),
        Sample(
            AuthorAffiliation(
                author="Sepidehsadat Hosseini", affiliations=["RBC Borealis"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Sepidehsadat_Hosseini2"],
        ),
        Sample(
            AuthorAffiliation(author="Greg Mori", affiliations=["SFU"], email=None),
            profile_urls=[
                "https://openreview.net/profile?id=~Greg_Mori1",
                "https://openreview.net/profile?id=~Greg_Mori2",
            ],
        ),
        Sample(
            AuthorAffiliation(
                author="Greg Mori", affiliations=["Simon Fraser University"], email=None
            ),
            profile_urls=[
                "https://openreview.net/profile?id=~Greg_Mori1",
                "https://openreview.net/profile?id=~Greg_Mori2",
            ],
        ),
        Sample(
            AuthorAffiliation(author="Greg Mori", affiliations=["RBC Borealis"], email=None),
            profile_urls=[
                "https://openreview.net/profile?id=~Greg_Mori1",
                "https://openreview.net/profile?id=~Greg_Mori2",
            ],
        ),
        Sample(
            AuthorAffiliation(author="Greg Mori", affiliations=["RBC Borealis", "SFU"], email=None),
            profile_urls=[
                "https://openreview.net/profile?id=~Greg_Mori1",
                "https://openreview.net/profile?id=~Greg_Mori2",
            ],
        ),
        Sample(
            AuthorAffiliation(author="Taylor Mordan", affiliations=["MobiLysis"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Taylor_Mordan1"],
        ),
        Sample(
            AuthorAffiliation(author="Taylor Mordan", affiliations=["VITA, EPFL"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Taylor_Mordan1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Taylor Mordan", affiliations=["MLIA, Sorbonne Université"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Taylor_Mordan1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Matthieu Cord", affiliations=["Sorbonne Université"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Matthieu_Cord1"],
        ),
        Sample(
            AuthorAffiliation(author="Matthieu Cord", affiliations=["Valeo"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Matthieu_Cord1"],
        ),
        Sample(
            AuthorAffiliation(author="Patrick Perez", affiliations=["Kyutai"], email=None),
            profile_urls=[
                "https://openreview.net/profile?id=~Patrick_Perez1",
                "https://openreview.net/profile?id=~Patrick_Perez2",
            ],
        ),
        Sample(
            AuthorAffiliation(author="Patrick Perez", affiliations=["Valeo"], email=None),
            profile_urls=[
                "https://openreview.net/profile?id=~Patrick_Perez1",
                "https://openreview.net/profile?id=~Patrick_Perez2",
            ],
        ),
        Sample(
            AuthorAffiliation(author="David Picard", affiliations=["LIGM, ENPC"], email=None),
            profile_urls=["https://openreview.net/profile?id=~David_Picard1"],
        ),
        Sample(
            AuthorAffiliation(author="Nazanin Mehrasa", affiliations=["SFU"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Nazanin_Mehrasa2"],
        ),
        Sample(
            AuthorAffiliation(author="Nazanin Mehrasa", affiliations=["Borealis AI"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Nazanin_Mehrasa2"],
        ),
        Sample(
            AuthorAffiliation(author="Jiawei He", affiliations=["SFU"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Jiawei_He1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Jiawei He", affiliations=["Simon Fraser University"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Jiawei_He1"],
        ),
        Sample(
            AuthorAffiliation(author="Jiawei He", affiliations=["Borealis AI"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Jiawei_He1"],
        ),
        Sample(
            AuthorAffiliation(author="Jiawei He", affiliations=["RBC Borealis"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Jiawei_He1"],
        ),
        Sample(
            AuthorAffiliation(author="Jeff Dean", affiliations=["Google"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Jeff_Dean1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Yoshua Bengio", affiliations=["Université de Montréal"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Yoshua_Bengio1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Geoffrey Hinton", affiliations=["University of Toronto"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Geoffrey_Hinton1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Yann LeCun", affiliations=["New York University"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Yann_LeCun1"],
        ),
        Sample(
            AuthorAffiliation(author="Ilya Sutskever", affiliations=["OpenAI"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Ilya_Sutskever2"],
        ),
        Sample(
            AuthorAffiliation(author="Oriol Vinyals", affiliations=["Google DeepMind"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Oriol_Vinyals1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Percy Liang", affiliations=["Stanford University"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Percy_Liang1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Chelsea Finn", affiliations=["Stanford University"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Chelsea_Finn1"],
        ),
        Sample(
            AuthorAffiliation(author="Hugo Larochelle", affiliations=["Google Brain"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Hugo_Larochelle1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Ian Goodfellow", affiliations=["Google DeepMind"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Ian_Goodfellow1"],
        ),
        Sample(
            AuthorAffiliation(author="Pieter Abbeel", affiliations=["UC Berkeley"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Pieter_Abbeel2"],
        ),
        # --- Authors from Asian universities with verified OpenReview profiles ---
        Sample(
            AuthorAffiliation(author="Jun Zhu", affiliations=["Tsinghua University"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Jun_Zhu2"],
        ),
        Sample(
            AuthorAffiliation(
                author="Shanghang Zhang", affiliations=["Peking University"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Shanghang_Zhang4"],
        ),
        Sample(
            AuthorAffiliation(
                author="Baobao Chang", affiliations=["Peking University"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Baobao_Chang1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Masashi Sugiyama", affiliations=["University of Tokyo"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Masashi_Sugiyama1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Tatsuya Harada", affiliations=["University of Tokyo"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Tatsuya_Harada1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Issei Sato", affiliations=["University of Tokyo"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Issei_Sato2"],
        ),
        Sample(
            AuthorAffiliation(author="Juho Lee", affiliations=["KAIST"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Juho_Lee2"],
        ),
        Sample(
            AuthorAffiliation(
                author="Ajit Rajwade",
                affiliations=["Indian Institute of Technology Bombay"],
                email=None,
            ),
            profile_urls=["https://openreview.net/profile?id=~Ajit_Rajwade1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Shanghang Zhang", affiliations=["Carnegie Mellon University"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Shanghang_Zhang4"],
        ),
        Sample(
            AuthorAffiliation(author="Juho Lee", affiliations=["POSTECH"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Juho_Lee2"],
        ),
        # --- Authors from European universities with verified OpenReview profiles ---
        Sample(
            AuthorAffiliation(author="Andreas Krause", affiliations=["ETH Zurich"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Andreas_Krause1"],
        ),
        Sample(
            AuthorAffiliation(author="Melanie Zeilinger", affiliations=["ETH Zurich"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Melanie_Zeilinger1"],
        ),
        Sample(
            AuthorAffiliation(author="Siddhartha Mishra", affiliations=["ETH Zurich"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Siddhartha_Mishra1"],
        ),
        Sample(
            AuthorAffiliation(author="Volkan Cevher", affiliations=["EPFL"], email=None),
            profile_urls=["https://openreview.net/profile?id=~Volkan_Cevher1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Yee Whye Teh", affiliations=["University of Oxford"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Yee_Whye_Teh2"],
        ),
        Sample(
            AuthorAffiliation(
                author="Tom Rainforth", affiliations=["University of Oxford"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Tom_Rainforth1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Yingzhen Li", affiliations=["Imperial College London"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Yingzhen_Li1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Isabel Valera", affiliations=["Saarland University"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Isabel_Valera1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Philipp Hennig", affiliations=["University of Tübingen"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Philipp_Hennig1"],
        ),
        Sample(
            AuthorAffiliation(
                author="Matthias Bethge", affiliations=["University of Tübingen"], email=None
            ),
            profile_urls=["https://openreview.net/profile?id=~Matthias_Bethge1"],
        ),
        # --- Authors confirmed to have no OpenReview profile ---
        Sample(
            AuthorAffiliation(author="Aaaaaaaa Bbbbbbbbb", affiliations=["University"], email=None),
            profile_urls=[],
        ),
        Sample(
            AuthorAffiliation(
                author="Linus Torvalds", affiliations=["Linux Foundation"], email=None
            ),
            profile_urls=[],
        ),
        Sample(
            AuthorAffiliation(author="Satya Nadella", affiliations=["Microsoft"], email=None),
            profile_urls=[],
        ),
        Sample(
            AuthorAffiliation(author="Jensen Huang", affiliations=["NVIDIA"], email=None),
            profile_urls=[],
        ),
        Sample(
            AuthorAffiliation(
                author="Marc Andreessen", affiliations=["Andreessen Horowitz"], email=None
            ),
            profile_urls=[],
        ),
        Sample(
            AuthorAffiliation(author="Patrick Collison", affiliations=["Stripe"], email=None),
            profile_urls=[],
        ),
        Sample(
            AuthorAffiliation(author="Sam Altman", affiliations=["OpenAI"], email=None),
            profile_urls=[],
        ),
        Sample(
            AuthorAffiliation(author="Elon Musk", affiliations=["Tesla"], email=None),
            profile_urls=[],
        ),
        Sample(
            AuthorAffiliation(author="Reed Hastings", affiliations=["Netflix"], email=None),
            profile_urls=[],
        ),
        Sample(
            AuthorAffiliation(author="Brian Chesky", affiliations=["Airbnb"], email=None),
            profile_urls=[],
        ),
    ]


def _evaluate_sample(index: int, sample: Sample) -> tuple[int, str, str, str, str, str]:
    """Evaluate a single sample by calling
    `find_openreview_profile_url`.

    Two distinct evaluation modes apply depending on whether the author is
    expected to have an OpenReview profile:

    - If `sample.profile_urls` is non-empty, a PASS requires the returned
      URL to match any one of the expected URLs. A single match is sufficient.
    - If `sample.profile_urls` is empty, the author has no OpenReview profile
      and a PASS requires the function to return None. Returning any URL is
      a false positive and is reported as UNEXPECTED.

    Args:
        index:  The original index of the sample in the dataset, used to
                reassemble results in order after concurrent execution.
        sample: The `Sample` instance to evaluate.

    Returns:
        A tuple of (index, author, affiliation, result_label, got_url, expected_urls).
    """
    affiliation_str = ", ".join(sample.author.affiliations)
    expected_str = ", ".join(sample.profile_urls) if sample.has_profile else "[dim]none[/dim]"

    url = find_openreview_profile_url(sample.author)
    normalised = normalise_openreview_url(url) if url else None

    if not sample.has_profile:
        if normalised is None:
            return (
                index,
                sample.author.author,
                affiliation_str,
                "[bold green]PASS[/bold green]",
                "[dim]—[/dim]",
                expected_str,
            )
        return (
            index,
            sample.author.author,
            affiliation_str,
            "[bold red]UNEXPECTED[/bold red]",
            f"[red]{normalised}[/red]",
            expected_str,
        )

    if normalised is None:
        return (
            index,
            sample.author.author,
            affiliation_str,
            "[bold red]MISSING[/bold red]",
            "[dim]—[/dim]",
            expected_str,
        )

    if normalised in sample.profile_urls:
        return (
            index,
            sample.author.author,
            affiliation_str,
            "[bold green]PASS[/bold green]",
            normalised,
            expected_str,
        )

    return (
        index,
        sample.author.author,
        affiliation_str,
        "[bold yellow]MISMATCH[/bold yellow]",
        f"[yellow]{normalised}[/yellow]",
        expected_str,
    )


def _build_results_table(
    results: list[tuple[str, str, str, str, str]],
) -> tuple[Table, int, int, int, int]:
    """Build a Rich table from evaluation results.

    Args:
        results: List of (author, affiliation, result_label, expected_urls, got_url)
                 tuples as returned by `_run_evaluation`.

    Returns:
        A tuple of (table, correct, missing, mismatch, unexpected) where the
        counts reflect the number of each outcome across all results.
    """
    table = Table(title="OpenReview Profile URL Evaluation", show_lines=True)
    table.add_column("Author", style="cyan")
    table.add_column("Affiliation", style="dim")
    table.add_column("Result", justify="center")
    table.add_column("Expected URLs", style="dim", no_wrap=False)
    table.add_column("Got URL", no_wrap=False)

    correct = missing = mismatch = unexpected = 0

    for author, affiliation, result_label, expected, got in results:
        if "PASS" in result_label:
            correct += 1
            got_styled = f"[green]{got}[/green]"
        elif "MISSING" in result_label:
            missing += 1
            got_styled = f"[red]{got}[/red]"
        elif "UNEXPECTED" in result_label:
            unexpected += 1
            got_styled = f"[red]{got}[/red]"
        else:
            mismatch += 1
            got_styled = f"[red]{got}[/red]"

        table.add_row(author, affiliation, result_label, expected, got_styled)

    return table, correct, missing, mismatch, unexpected


def _run_evaluation(
    samples: list[Sample],
    max_workers: int,
) -> list[tuple[str, str, str, str, str]]:
    """Run evaluation concurrently and return results in original sample
    order.

    Args:
        samples:     List of `Sample` instances to evaluate.
        max_workers: Maximum number of concurrent threads.

    Returns:
        A list of (author, affiliation, result_label, expected_urls, got_url)
        tuples in the same order as the input samples.
    """
    results = [None] * len(samples)

    with make_progressbar() as progress:
        task = progress.add_task("Evaluating samples", total=len(samples))
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(_evaluate_sample, i, sample): i for i, sample in enumerate(samples)
            }
            for future in as_completed(futures):
                index, author, affiliation, result_label, got, expected = future.result()
                results[index] = (author, affiliation, result_label, expected, got)
                progress.advance(task)

    return results


def evaluate(samples: list[Sample], max_workers: int = 4) -> None:
    """Evaluate the accuracy of `find_openreview_profile_url` on a
    dataset.

    For each sample, calls `find_openreview_profile_url` and checks whether
    the result matches expectations. Two evaluation modes apply:

    - Authors with `profile_urls` non-empty: PASS if the returned URL
      matches any expected URL, MISSING if None is returned, MISMATCH if
      a wrong URL is returned.
    - Authors with `profile_urls=[]`: PASS if None is returned (no false
      positive), UNEXPECTED if any URL is returned.

    Samples are processed concurrently using a thread pool. Results are
    displayed in a Rich table in the original sample order, followed by
    a summary.

    Args:
        samples:     A list of `Sample` instances to evaluate.
        max_workers: Maximum number of concurrent threads. Defaults to 4.
    """
    results = _run_evaluation(samples, max_workers=max_workers)
    table, correct, missing, mismatch, unexpected = _build_results_table(results)

    console.print(table)
    console.print(
        f"\n[bold]Results:[/bold] "
        f"[green]{correct}/{len(samples)} correct[/green], "
        f"[red]{missing} missing[/red], "
        f"[yellow]{mismatch} mismatch[/yellow], "
        f"[red]{unexpected} unexpected[/red]"
    )


def main() -> None:
    r"""Define the main function."""
    samples = make_dataset()
    evaluate(samples)


if __name__ == "__main__":
    load_dotenv()

    main()
