...
def extract_text_pdfplumber(
    ...
    r"""Extract text from a digital PDF using pdfplumber.

    Iterates through the pages of the PDF sequentially and extracts the
    text content of each page using pdfplumber's layout-sensitive extraction.
    Pages with no extractable text (e.g. scanned images) are silently
    skipped. Suitable for digitally-created PDFs where text is embedded
